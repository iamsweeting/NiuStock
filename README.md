...
| 功能 | 来源 | 说明 |
| --- | --- | --- |
| 📈 趋势 | 牛门线分析（Kivy/KivyMD） | 压力/止盈/止损线（唐奇安通道 + ATR），最近 **10 个交易日** K 线 + 文字判读 |
...
- ⏱ **盘中处理**：枢轴点所选日=今天且处于交易时段（北京时间 9:15~15:00）时，
  ...
  趋势图表保留盘中实时K线
- 📊 **趋势页 10 个交易日窗口**：YL 压力线 / QL 止盈线 / ZS 止损线（+CBX20/CBX60 成本线）5 色指标；
  **版本可选**（基础主图版 / 标的版 / 指数版），按代码自动默认、用户可切换
...
趋势线本质是**唐奇安通道（Donchian Channel）+ ATR 波动率**的复合变形指标：

| 指标线 | 公式 | 含义 |
| --- | --- | --- |
| YL（压力线） | `REF(HHV(H,20),1) + 0.5×ATR(14)` | 前 20 日最高价 + 0.5 倍平均真实波幅，突破入场 / 压力线 |
| QL（止盈线） | `REF(HHV(H,20),1) + 1.0×ATR(14)` | 前 20 日最高价 + 1 倍 ATR，强压力 / 止盈线 |
| ZS（止损线） | `MA(C,10)` | 10 日均线，趋势参考 / 止损线 |
...
│   ├── api.py                 # 统一数据源（腾讯→新浪）+ 趋势/枢轴点获取与聚合
│   ├── indicator.py           # 趋势指标计算（YL/QL/ZS + 成本线，纯函数）
│   ├── interpreter.py         # 趋势判读（纯函数）
