...
| 🏦 行情 | 新增功能 | 本日实时（两市成交额/预测额/指数行情/沪深300中位数）+ 近5日历史（成交额/中间价/WTI/伦敦金） |
...
- 🗂 **四页底部导航**：牛门线 / 枢轴点 / 批量枢轴点 / 行情，一键切换
- 🏦 **行情页**（切换页面自动刷新，TTL 90 秒 + 请求节流，低请求量防反爬）：
  - 本日实时：两市成交额 + **本日预测额**（按已交易时长线性外推）、
    上证/中证A500/中证1000/沪深300/恒生/恒生科技/伦敦金/沪金等指数行情、
    **沪深300中位数**（成分股价格中位数）+ **沪深300中位数市盈率(TTM)**（乐咕乐股，置页尾）
  - 近 5 个交易日：两市成交额、美元兑人民币中间价、WTI原油、伦敦金
  - 数据源：新浪（批量行情/成交额历史兜底）、乐咕乐股（中位数PE）、
    中国货币网（中间价）、东方财富（两市成交额历史，失败自动回退新浪），各小节独立容错；
    （韩国KOSPI/日经/韩国半导体依赖 Yahoo，手机网络不可达，已按需求移除）
- 🌐 **统一数据源**（自动切换，不显示切换按钮）：
...
- 🔍 **查询名单快捷按钮**：快捷区显示**名单前 3 个**，默认预置
  半导体设备ETF（159516）、中韩半导体ETF（513310）、中证1000ETF（159845）；
